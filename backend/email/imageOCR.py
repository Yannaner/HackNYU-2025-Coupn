...
def extract_promo_codes(text_blocks):
    ...
    promo_indicators = [
        'code', 'coupon', 'promo', 'use', 'enter', 'discount',
        'save', 'off', 'extra', 'special', 'offer'
    ]
...
def classify_words(image_path):
    ...
    # Extract potential promo codes
    promo_codes = extract_promo_codes(results)
    
    # Concatenate all text
    full_text = ' '.join([text for (bbox, text, prob) in results])
    
    # Add promo code context if found
    if promo_codes:
        promo_context = " [DETECTED_PROMO_CODES: " + ", ".join(promo_codes) + "]"
        full_text += promo_context
